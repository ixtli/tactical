import generateDebounce from "./debounce.js";
import { TILE_HEIGHT, TILE_WIDTH } from "./tile_geometry.js";
import TWEEN from "./Tween.js";
import { emit, emitb } from "./bus.js";
import Chunk from "./map/chunk.js"; // jshint ignore:line
...
export default function Engine(container, options) {
  ...
  * @private
  */
	this._camera = new THREE.OrthographicCamera(0, 0, 0, 0, CAMERA_NEAR, CAMERA_FAR);
  ...
  * @private
  */
	this._selectionPlaneMesh = new THREE.Mesh(new THREE.PlaneBufferGeometry(SELECT_SIZE, SELECT_SIZE), new THREE.MeshBasicMaterial({ color: 0x202020, visible: false }));
...
Engine.prototype._setupScene = function () {
	...
	this._pickingTexture = new THREE.WebGLRenderTarget(this._windowWidth, this._windowHeight);
...
Engine.prototype.useMap = function (newMap) {
	...
	this._currentMapPickingSceneObject = new THREE.Mesh(pickingGeom, pickingMaterial);
	...
	const geom = new THREE.BoxBufferGeometry(newMap.width(), newMap.height(), newMap.depth());
...
Engine.prototype.resetCamera = function () {
	...
	this.lookAt(new THREE.Vector3(Math.floor(map.width() / 2), 0, Math.floor(map.depth() / 2)), 0);
...
Engine.prototype._pick = function () {
	...
	this._renderer.readRenderTargetPixels(this._pickingTexture, this._nextPickCoordinates.x, this._pickingTexture.height - this._nextPickCoordinates.y, 1, 1, PICK_PIXEL_BUFFER);

	this._pickStateDirty = false;
	//interpret the pixel as an ID
	let id = PICK_PIXEL_BUFFER[0] << 16 | PICK_PIXEL_BUFFER[1] << 8 | PICK_PIXEL_BUFFER[2];
...
Engine.prototype.zoom = function (newFactor, ms) {
	...
	if (ms < 1) {
		...
		this._zoomTween = new TWEEN.Tween(this).to({ _zoom: this._zoomTarget }, ms).onUpdate(this._updateCameraFrustum.bind(this)).onComplete(emitb("engine.camera.zoom.end", [ms])).start();
	}
};
...
Engine.prototype.lookAt = function (target, ms) {
	...
	if (ms < 1) {
		...
		this._panTween = new TWEEN.Tween(this._camera.position).to(pos).easing(TWEEN.Easing.Quintic.Out).onComplete(emitb("engine.camera.pan.end", null)).start();
	}
};
...
Engine.prototype.orbit = function (ms, reverse) {
	...
	this._orbitTween = new TWEEN.Tween(this).to({ _cameraOrbitDegrees: ot }, ms).onUpdate(() => {
