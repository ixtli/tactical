import Engine, { EAST, NORTH, SOUTH, WEST } from "./engine.js"; // jshint ignore:line
import { emit, subscribe, unsubscribe } from "./bus.js";
import generateFSM, { START } from "./state_machine.js";
...
export default function SelectionManager(graphicsEngine) {
  ...
  * @private
  */
	this._highlightBoxGeometry = new THREE.BoxBufferGeometry(TILE_WIDTH, TILE_HEIGHT, TILE_WIDTH);
  ...
  * @private
  */
	this._primarySelectionBox = new THREE.Mesh(this._highlightBoxGeometry, PRIMARY_SELECTION_BOX_MATERIAL);
  ...
  * @private
  */
	this._secondarySelectionBox = new THREE.Mesh(this._highlightBoxGeometry, SECONDARY_SELECTION_BOX_MATERIAL);
  ...
  * @private
  */
	this._highlightBox = new THREE.Mesh(this._highlightBoxGeometry, HIGHLIGHT_BOX_MATERIAL);
  ...
  * @private
  */
	this._selectionVolume = new THREE.Mesh(new THREE.BoxBufferGeometry(1, 1, 1), SELECTION_VOLUME_MATERIAL);
...
SelectionManager.prototype._updateSelectionVolume = function () {
	...
	this._selectionVolume.geometry.attributes.position.set(geo.attributes.position.array);
	this._selectionVolume.geometry.attributes.position.needsUpdate = true;
	this._selectionVolume.visible = true;

	this._selectionVolume.position.set(Math.min(p.x, s.x) + w / 2 - (TILE_WIDTH + OFFSET) / 2, Math.min(p.y, s.y) + h / 2 - (TILE_HEIGHT + OFFSET) / 2, Math.min(p.z, s.z) + d / 2 - (TILE_WIDTH + OFFSET) / 2);
...
SelectionManager.prototype.selectSecondary = function (vec) {
	...
	this._updateSelectionVolume();
	emit("select.secondary.tile", [this._primarySelection, this._secondarySelection]);
...
SelectionManager.prototype.selectPrimary = function (vec) {
	...
	this._updateSelectionVolume();
	emit("select.primary.tile", [this._primarySelection, this._secondarySelection]);
...
SelectionManager.prototype.deselectSecondary = function () {
	...
	this._secondarySelection = null;
	this._updateSelectionVolume();
	emit("select.secondary.deselect", [this._primarySelection, this._secondarySelection]);
};
...
SelectionManager.prototype.deselectPrimary = function () {
	...
	this._primarySelection = null;
	this._updateSelectionVolume();
	emit("select.primary.deselect", [this._primarySelection, this._secondarySelection]);
};
