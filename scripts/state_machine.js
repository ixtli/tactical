...
export default function generateFSM(context, stateMap, name, initial) {
  ...
  */
	let ret = newState => {
		...
		if (current !== null) {
			console.assert(current.to.has(newState), `${name}: Can't transition from ${cName} to ${tName}`);
		}

		if (target !== null) {
			console.assert(target.from.has(currentState), `${name}: Can't transition to ${tName} from ${cName}`);
		}
